...
def learn_pcfg_res(file_input=None):
    if file_input is None: bank = treebank.tagged_sents()[:1]
...
def parse_viterbi_with_pcfg (grammar, sentence, flag_pred):
...
for i in sentences:
    ...
    probab_pred = parse_viterbi_with_pcfg(grammar_pred, i, 1)
